...
| Change | Detail |
|---|---|
| Build system | `CMakeLists.txt`, `src/CMakeLists.txt` (516 lines), `CMakePresets.json` — moves to CMake |
...
non-functional dummy backend and MSVC is the standard advice. [unverified — from Qt forum consensus,
not measured; **this is the single most important thing to verify empirically**, see Phase 1]
...
| Costs | vcpkg + protobuf, VC++ runtime, install-layout break, exe-only upgrades invalid | A Qt module build per CI run (already cached) |
...
[verified] — and mingw-w64 has shipped `windows.devices.bluetooth.h` since **2023-07-13** [verified
...
**Phase 1 — Answer the one question that decides C1 vs C2. (~half a day, no port)**
Install Qt 6.8.2 `win64_mingw` via aqtinstall in CI and check whether `Qt6Bluetooth` is the real
backend or the dummy — the cheapest probe is a job that builds a five-line program calling
`QBluetoothLocalDevice::allDevices()` and logs `qt.bluetooth.windows`. If it is the dummy, try C2:
build qtconnectivity 6.8.2 from source with the existing CI step and see whether `TEST_winrt_bt`
passes under the MSYS2 toolchain. **If C2 works, take it. If not, C1 and accept the MSVC costs.**
Nothing is ported either way.
...
Apply the Part II substitutions, move to CMake, and take upstream qt6's non-device changes as
reference patches rather than merging its branch. Expect the real work to be `bluetooth.cpp`,
`homeform.cpp`, the virtual devices and `webserverinfosender.cpp`, not the device.
...
**The mingw/dummy-backend claim is unverified.** Everything in Part III's route choice rests on it,
and it comes from forum consensus rather than measurement. Phase 1 exists to settle it before any
port begins.
...
already found the Win32 descriptor patch has no WinRT equivalent need. The `/* QZ rviola` edit does
...
- Does official Qt 6.8.2 mingw ship a working Bluetooth backend, or the dummy? **Decides C1 vs C2.**
